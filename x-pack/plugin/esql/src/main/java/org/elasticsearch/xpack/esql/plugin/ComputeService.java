...
public class ComputeService {
    ...
    public void execute(
        ...
        try {
            ...
            // Catch broadly rather than just EsqlIllegalArgumentException. buildSubPlan's invariant guards all raise that type, but the
            // tree walk it performs (transformDownSkipBranch, replaceChildren, output()) can surface any runtime exception, and
            // execute() runs downstream of EsqlSession's analyzed-plan callback - a synchronous throw there can be absorbed by the
            // completing listener and leave the request hanging instead of failing it.
        ...
        switch (executionPlan) {
            ...
            case SubPlan.Merge merge -> new SubPlansExecutor(
                ...
                execInfo,
                initialClusterStatuses
            ).execute(merge, planTimeProfile, listener);
            default -> throw new IllegalStateException("unexpected SubPlan type: " + executionPlan.getClass());
        }
    }

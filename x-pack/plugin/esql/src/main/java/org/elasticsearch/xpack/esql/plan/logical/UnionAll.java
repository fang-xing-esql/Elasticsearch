...
public class UnionAll extends Fork implements PostOptimizationPlanVerificationAware {
    ...
    public static void checkTotalBranchCount(LogicalPlan optimizedPlan, int maxBranches, Failures failures) {
        ...
        if (branches > maxBranches) {
            ...
            // Deliberately does not say "subqueries": branches also come from a FROM pattern expanding to several sources, from view
            // expansion, and from PromQL, none of which the user wrote as a subquery. Name the limit so it is discoverable - it is the
            // max_query_branches query pragma - while noting it is not adjustable everywhere, since pragmas need accept_pragma_risks
            // outside snapshot builds (see EsqlQueryRequest#validate).
            failures.add(
                Failure.fail(
                    unionAlls.getFirst(),

 ...
 */
public class RoundToLinear extends EsqlScalarFunction {
    public static final NamedWriteableRegistry.Entry ENTRY =
        new NamedWriteableRegistry.Entry(Expression.class, "RoundTo", RoundToLinear::new);
    ...
    private RoundToLinear(StreamInput in) throws IOException {
        this(
    ...
    public final Expression replaceChildren(List<Expression> newChildren) {
        return new RoundToLinear(source(), newChildren.get(0), newChildren.subList(1, newChildren.size()));
    }
    ...
    private static final Map<DataType, Build> SIGNATURES = Map.ofEntries(
        Map.entry(DATETIME, RoundToLong.LINEAR_BUILD),
        Map.entry(DATE_NANOS, RoundToLong.LINEAR_BUILD),
        Map.entry(INTEGER, RoundToInt.LINEAR_BUILD),
        Map.entry(LONG, RoundToLong.LINEAR_BUILD),
        Map.entry(DOUBLE, RoundToDouble.LINEAR_BUILD)
    );
}

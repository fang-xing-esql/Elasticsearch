 ...
 */
public class RoundToManual extends EsqlScalarFunction {
    public static final NamedWriteableRegistry.Entry ENTRY =
        new NamedWriteableRegistry.Entry(Expression.class, "RoundTo", RoundToManual::new);
    ...
    private RoundToManual(StreamInput in) throws IOException {
        this(
    ...
    public final Expression replaceChildren(List<Expression> newChildren) {
        return new RoundToManual(source(), newChildren.get(0), newChildren.subList(1, newChildren.size()));
    }
    ...
    private static final Map<DataType, Build> SIGNATURES = Map.ofEntries(
        Map.entry(DATETIME, RoundToLong.MANUAL_BUILD),
        Map.entry(DATE_NANOS, RoundToLong.MANUAL_BUILD),
        Map.entry(INTEGER, RoundToInt.MANUAL_BUILD),
        Map.entry(LONG, RoundToLong.MANUAL_BUILD),
        Map.entry(DOUBLE, RoundToDouble.MANUAL_BUILD)
    );
}

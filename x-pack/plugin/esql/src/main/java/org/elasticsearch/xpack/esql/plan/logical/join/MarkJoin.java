...
public class MarkJoin extends AbstractSubqueryJoin {
    ...
    /**
     * The mark attribute is generated by this join — it is not a requirement on the input, so it must not appear in
     * {@code references()}. Without this override the base {@link org.elasticsearch.xpack.esql.plan.QueryPlan#computeReferences()}
     * would include it (via {@code nodeInfo} expressions), causing pre-analysis field collection (e.g.
     * {@link org.elasticsearch.xpack.esql.session.FieldNameUtils}) to request the synthetic {@code $$mark} name from the index.
     */
    @Override

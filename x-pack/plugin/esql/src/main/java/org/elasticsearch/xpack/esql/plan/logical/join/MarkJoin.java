...
public class MarkJoin extends AbstractSubqueryJoin {
     ...
     *       {@link InSubqueryResolver#SYNTHETIC_MARK_PREFIX} name prefix. {@code CombineEvals} may merge the mark
    ...
    static boolean isInlinedBoundary(LogicalPlan plan) {
        if (plan instanceof Eval eval) {
            return eval.fields().isEmpty() == false
                && eval.fields().stream().anyMatch(alias -> alias.name().startsWith(InSubqueryResolver.SYNTHETIC_MARK_PREFIX));
        }

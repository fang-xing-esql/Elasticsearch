...
public class SubqueryTests extends AbstractStatementParserTests {
    ...
    public void testSubqueryWithAllProcessingCommandsInMainquery() {
        assumeTrue("Requires subquery in FROM command support", EsqlCapabilities.Cap.SUBQUERY_IN_FROM_COMMAND.isEnabled());
        assumeTrue("Requires inline stats in FROM command support", EsqlCapabilities.Cap.INLINE_STATS.isEnabled());
        ...
        var joinIndexPattern = randomIndexPattern(without(CROSS_CLUSTER), without(WILDCARD_PATTERN), without(INDEX_SELECTOR));
        String query = LoggerMessageFormat.format(null, """

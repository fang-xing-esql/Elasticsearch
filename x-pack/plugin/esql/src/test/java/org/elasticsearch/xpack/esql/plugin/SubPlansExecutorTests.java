...
public class SubPlansExecutorTests extends ESTestCase {
    ...
    public void testOneMergeEightLeafRandomLeafFailureInExecutePlan() throws Exception {
        ...
        assertEquals("injected leaf failure", ex.getCause().getMessage());
        assertTrue("cancelQueryOnFailure must have fired", cancelled.get());
        assertTrue("exchange service must be fully empty after leaf failure", exchangeService.isEmpty());
    }
    ...
    public void testNestedMergesRandomFailureInRunCompute() {
        ...
        assertTrue("cancelQueryOnFailure must have fired", cancelled.get());
        assertTrue("exchange service must be fully empty after nested failure", exchangeService.isEmpty());
    }
    ...
    public void testNestedMergesRandomLeafFailureInExecutePlan() throws Exception {
        ...
        assertEquals("injected leaf failure", ex.getCause().getMessage());
        assertTrue("cancelQueryOnFailure must have fired", cancelled.get());
        assertTrue("exchange service must be fully empty after nested leaf failure", exchangeService.isEmpty());
    }
    ...
    public void testCancellationWithManyLeavesDoesNotStackOverflow() throws Exception {
        ...
        assertTrue("cancelQueryOnFailure must have fired", cancelled.get());
        assertTrue("exchange service must be fully empty after cancellation", exchangeService.isEmpty());
    }
    ...
    public void testCompleteManyLeavesDoNotStackOverflow() throws Exception {
        ...
        future.get();
        assertFalse("cancelQueryOnFailure must not fire on success", cancelled.get());
        assertTrue("exchange service must be fully empty after success", exchangeService.isEmpty());
    }
    ...
    public void testExecutorRejectionDrainsRemainingLeaves() throws Exception {
        ...
        assertTrue("cancelQueryOnFailure must have fired", cancelled.get());
        assertTrue("exchange service must be fully empty after executor rejection", exchangeService.isEmpty());
    }
    ...
    public void testUndispatchedLeavesHaveNoSinkHandlers() throws Exception {
        ...
        future.get();
        assertFalse("cancelQueryOnFailure must not fire on success", cancelled.get());
        assertBusy(() -> assertTrue("exchange service must be fully empty after success", exchangeService.isEmpty()));
    }
    ...
    public void testSynchronousExecutePlanFailureFirstWaveFailsCleanly() {
        ...
        assertTrue("cancelQueryOnFailure must have fired", cancelled.get());
        assertTrue("exchange service must be fully empty after synchronous failure", exchangeService.isEmpty());
    }
    ...
    public void testSynchronousExecutePlanFailureOnRefillWaveFailsCleanly() throws Exception {
        ...
        assertTrue("cancelQueryOnFailure must have fired", cancelled.get());
        assertBusy(() -> assertTrue("exchange service must be fully empty after refill failure", exchangeService.isEmpty()));
    }
    ...
    public void testTwoExecutorsWithSameSessionIdDoNotCollide() throws Exception {
        ...
        assertBusy(() -> assertTrue("exchange service must be fully empty once all sinks drain", exchangeService.isEmpty()));
    }

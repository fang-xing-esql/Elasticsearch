...
public class LogicalPlanOptimizerSubqueryGoldenTests extends GoldenTestCase {
    ...
    /**
     * An unbounded {@code SORT} in one branch of a nested union must bound only that branch - it becomes a {@code TopN} inside the
     * branch. Bounding the enclosing branch instead would put an implicit {@code Limit} above the nested {@code UnionAll} and truncate
     * the concatenation of <em>all</em> of its branches, losing rows the equivalent flat query returns. So the expected plan has no
     * {@code Limit} sitting directly above a {@code UnionAll}.
     */
    public void testUnboundedSortInNestedBranchDoesNotLimitTheNestedUnion() {
        runGoldenTest("""
            FROM (FROM (FROM employees | SORT emp_no), (FROM employees | LIMIT 10)), (FROM languages)
            | STATS c = COUNT(*)
            """, STAGES);
    }

    /**
     * The same two nested branches as {@link #testUnboundedSortInNestedBranchDoesNotLimitTheNestedUnion}, written in the other order.
     * Union all is commutative, so the two expected plans must agree on where limits sit - compare the files.
     * <p>
     * This half is the reference, not the guard: it already produced the correct plan before the fix. The bug was order-dependent
     * because the unbounded-sort search walked the whole branch subtree with one shared "already saw a Limit" flag, and
     * {@code forEachDownMayReturnEarly} aborted the entire traversal on the first {@code Limit} - so with the bounded branch first the
     * sort was never seen and no implicit limit was added, while the other order truncated the whole inner union. Keeping both orders
     * means a future regression is caught whichever direction it leans.
     */
    public void testUnboundedSortInNestedBranchIsBranchOrderIndependent() {
        runGoldenTest("""
            FROM (FROM (FROM employees | LIMIT 10), (FROM employees | SORT emp_no)), (FROM languages)
            | STATS c = COUNT(*)
            """, STAGES);
    }

    /**
     * The positive counterpart of {@code LogicalPlanOptimizerSubqueryTests#testUnboundedSortInsideInSubqueryInUnionAllBranchIsRejected}:
     * once the {@code SORT} inside the {@code IN} subquery carries a {@code LIMIT}, the query is legal.
     * <p>
     * What the expected plan pins is where the bounding ends up. The {@code IN} becomes a {@code SemiJoin} whose right side is the
     * subquery, and the {@code SORT | LIMIT 5} there fuses into a {@code TopN[...,5]} on that side alone. No implicit {@code Limit}
     * appears above the {@code UnionAll}: {@code PushDownFilterAndLimitIntoUnionAll} walks both join sides looking for an unbounded
     * sort, and the right side's {@code Limit} is the first thing it meets on that path, so the search stops there rather than
     * treating the branch as needing to be bounded.
     */
    public void testBoundedSortInsideInSubqueryInUnionAllBranch() {
        runGoldenTest("""
            FROM (FROM employees | WHERE emp_no IN (FROM employees | SORT emp_no | LIMIT 5 | KEEP emp_no)), (FROM languages)
            | STATS c = COUNT(*)
            """, STAGES);
    }

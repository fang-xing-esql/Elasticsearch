...
public class LogicalPlanOptimizerSubqueryGoldenTests extends GoldenTestCase {
    ...
    public void testForksInsideAndAfterSubqueryInFrom() {
        runGoldenTest("""
            FROM (FROM employees
    ...
    public void testSubQueryInFromInsideView() {
        runGoldenTest(
            "FROM branching_view",
            STAGES,
            Map.of("branching_view", "FROM (FROM employees | LIMIT 10), (FROM employees | LIMIT 20)")
        );
    }

    public void testSubqueryInFromInsideViewReferencedBySubqueryInFrom() {
        runGoldenTest(
            "FROM (FROM branching_view | LIMIT 10), (FROM employees | LIMIT 10)",
            STAGES,
            Map.of("branching_view", "FROM (FROM employees | LIMIT 10), (FROM employees | LIMIT 20)")
        );
    }

    public void testNestedViewsWithSubqueryInFrom() {
        runGoldenTest(
            "FROM outer_view",
            STAGES,
            Map.of(
                ...
                "outer_view",
                "FROM branching_view | LIMIT 5"
            )
        );
    }

    public void testForkAfterViewInSubqueryInFrom() {
        runGoldenTest(
    ...
    public void testUnionAllSubqueriesReferencingMultipleDatasets() {
    ...
    private void runNestedHeavyGoldenTest(String query) {
        runNestedHeavyGoldenTest(query, Map.of());
    }

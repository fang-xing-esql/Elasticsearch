...
public class BooleanExpressionGeneratorTests extends ESTestCase {
    ...
    /**
     * A bare {@code BY field} grouping is valid next to an IN-subquery filter. {@code BY alias = expr}
     * is not generated: that alias would shadow the child field and the filter would be rejected.
     */
    public void testStatsGeneratesInSubqueryInAggregateWhereWithBareFieldGrouping() {
        String command = null;
        for (int i = 0; i < 32; i++) {
            CommandGenerator.CommandDescription description = StatsGenerator.INSTANCE.generate(
                List.of(),
                COLUMNS,
                QUERY_SCHEMA,
                EXECUTOR,
            ...
            // Outer grouping is a suffix; an inner subquery STATS may also contain " by ".
            if (description.commandString().matches("(?s).* by (`?(number|word)`?)$")) {
                command = description.commandString();
                break;
            }
        }
        assertNotNull("expected a STATS command with a bare-field BY clause", command);
        assertTrue(command, command.startsWith(" | stats "));
        assertThat(command, containsString(" WHERE "));
        assertInSubquery(command);
        assertThat(command, matchesPattern("(?s).* by (`?(number|word)`?)$"));
    }
